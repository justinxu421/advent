...
def p1(a):
    ...
    while max(total_1, total_2) < 1000:
        ...
        c += 3
        if player == 1:
            start_1 = get_score(start_1, sum_)
            total_1 += start_1
            player = 2
        else:
            start_2 = get_score(start_2, sum_)
            total_2 += start_2
            player = 1
...
def wins(total_1, total_2, start_1, start_2, player):
    # print(total_1, total_2, start_1, start_2, player)
    if total_1 >= 21:
        return (1, 0)
    if total_2 >= 21:
        return (0, 1)
    ...
    for r1 in range(1, 4):
        for r2 in range(1, 4):
            for r3 in range(1, 4):
                sum_ = r1 + r2 + r3
                if player == 1:
                    score = get_score(start_1, sum_)
                    new_total = wins(total_1 + score, total_2, score, start_2, 2)
                elif player == 2:
                    score = get_score(start_2, sum_)
